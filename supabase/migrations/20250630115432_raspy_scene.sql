/*
  # Fix City Leaderboard Type Mismatch

  1. Changes
    - Drop and recreate get_city_leaderboard function to fix type mismatch
    - Update return type from numeric to double precision
    - Add explicit type casting for numeric values
    - Add pg_trgm extension for text similarity
    - Add city name normalization to improve matching

  2. Functions
    - get_or_create_city: Improved with name normalization
    - find_similar_cities: New function to find similar cities
    - get_city_leaderboard: Fixed with proper type handling
*/
...
DROP FUNCTION IF EXISTS get_city_leaderboard(uuid, text);
