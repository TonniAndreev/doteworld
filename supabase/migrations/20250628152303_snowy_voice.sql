/*
  # Walk History and Sessions Tables

  1. New Tables
    - `walk_sessions` - Tracks active and completed walk sessions
    - `walk_history` - Stores historical walk data with statistics
  
  2. Changes
    - Add `session_id` column to `walk_points` table
    - Create indexes for performance optimization
  
  3. Views
    - `walk_statistics` - Aggregated statistics by dog
    - `user_walk_statistics` - Aggregated statistics by user
...
-- RLS policies for walk_sessions - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
...
-- RLS policies for walk_history - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
