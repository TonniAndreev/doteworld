/*
  # Add path_coordinates to walk_points table
  
  1. New Columns
    - Add `path_coordinates` JSONB column to walk_points table to store arrays of coordinates
  
  2. Indexes
    - Create GIN index on path_coordinates for better query performance
  
  3. Functions
    - Create extract_coordinates function to parse the JSONB data
  
  4. Views
    - Create walk_points_expanded view for backward compatibility
*/
...
SELECT 
  ...
  wp.dog_id,
  wp.walk_session_id,
  wp.start_time, -- Changed from wp.timestamp to wp.start_time which exists in the table
  coords.latitude,
  coords.longitude
FROM 
  walk_points wp,
