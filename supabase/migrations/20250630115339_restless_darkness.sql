...
RETURNS TABLE (
  ...
  dog_name text,
  dog_photo_url text,
  territory_size double precision,  -- Changed from numeric to double precision
  total_distance double precision,  -- Changed from numeric to double precision
  badge_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH profile_stats AS (
    ...
    SELECT 
      pc.profile_id,
      SUM(pc.territory_size)::double precision AS territory_size  -- Explicit cast to double precision
  ...
  walk_stats AS (
    ...
    SELECT 
      pd.profile_id,
      SUM(ws.distance)::double precision AS total_distance  -- Explicit cast to double precision
  ...
  SELECT 
    ...
    di.dog_name,
    di.dog_photo_url,
    COALESCE(ps.territory_size, 0::double precision) AS territory_size,  -- Explicit cast in COALESCE
    COALESCE(ws.total_distance, 0::double precision) AS total_distance,  -- Explicit cast in COALESCE
